from app.users.application.exceptions import InvalidOldPasswordException, UnauthorizedException, UserNotFoundException
import pytest
...
@pytest.mark.asyncio
async def test_cambiar_contrasena_no_autorizado():
    """
    Tests that UnauthorizedException is raised when a non-authorized user tries to change a password.
    """
    # Arrange
    from app.users.application.repositories.i_user_repository import IUserRepository
    from app.users.application.policies.user_policy import UserPolicy
    ...
    dto = CambiarContrasenaDTO(contrasena_antigua="old_password", contrasena_nueva="NewPass123!@#")
    ...
    # Act & Assert
    with pytest.raises(UnauthorizedException, match="Not authorized to change this password."):
        await use_case.execute(mock_current_user, target_user_id, dto)
...
@pytest.mark.asyncio
async def test_cambiar_contrasena_usuario_no_encontrado():
    """
    Tests that UserNotFoundException is raised when the user is not found.
    """
    # Arrange
    from app.users.application.repositories.i_user_repository import IUserRepository
    from app.users.application.policies.user_policy import UserPolicy
    ...
    user_policy = Mock(spec=UserPolicy)
    user_policy.actualizar_perfil.return_value = True
...
@pytest.mark.asyncio
async def test_cambiar_contrasena_antigua_invalida():
    """
    Tests that InvalidOldPasswordException is raised when the old password is invalid.
    """
    # Arrange
    from app.users.application.repositories.i_user_repository import IUserRepository
    from app.users.application.policies.user_policy import UserPolicy
    ...
    mock_password_hasher = Mock(spec=IPasswordHasher)
    mock_password_hasher.verify.return_value = False # Old password is incorrect
    mock_password_hasher.hash.return_value = "hashed_new_password"
    ...
    # Act & Assert
    with pytest.raises(InvalidOldPasswordException, match="Invalid old password."):
        await use_case.execute(mock_user, user_id, dto)
