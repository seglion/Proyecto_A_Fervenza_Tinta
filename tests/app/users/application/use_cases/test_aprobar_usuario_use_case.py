...
def test_aprobar_usuario_use_case_file_exists():
    """
    Tests if the aprobar usuario use case file exists.
    """
    try:
        from app.users.application.use_cases import aprobar_usuario_use_case
    except ImportError:
        pytest.fail("Use case file does not exist: src/app/users/application/use_cases/aprobar_usuario_use_case.py")

def test_aprobar_usuario_use_case_class_exists():
    """
    Tests if the AprobarUsuarioUseCase class exists in the use case file.
    """
    try:
        from app.users.application.use_cases.aprobar_usuario_use_case import AprobarUsuarioUseCase
    except ImportError:
        pytest.fail("AprobarUsuarioUseCase class does not exist in aprobar_usuario_use_case.py")

@pytest.mark.asyncio
async def test_aprobar_usuario_exitoso():
    """
    Tests the successful approval of a user by an admin.
    """
    # Arrange
    admin_user = User(
        id=uuid4(),
    ...
    user_to_approve_id = uuid4()
    user_to_approve = User(
        id=user_to_approve_id,
    ...
    mock_user_repository.buscar_por_id = AsyncMock(return_value=user_to_approve)
    mock_user_repository.actualizar = AsyncMock(return_value=user_to_approve)

    mock_email_service = Mock(spec=IEmailService)
    mock_email_service.enviar_email_bienvenida = AsyncMock()
    ...
    # Act
    await use_case.execute(admin_user, user_to_approve_id)

    # Assert
    mock_user_repository.buscar_por_id.assert_called_once_with(user_to_approve_id)
...
async def test_aprobar_usuario_no_autorizado():
    """
    Tests that a ValueError is raised when a non-admin user tries to approve another user.
    """
    # Arrange
    non_admin_user = User(
        id=uuid4(),
    ...
    mock_email_service.enviar_email_bienvenida.assert_not_called()

@pytest.mark.asyncio
async def test_aprobar_usuario_no_encontrado():
    """
    Tests that a ValueError is raised when the user to approve is not found.
    """
    # Arrange
    admin_user = User(
        id=uuid4(),
...
@pytest.mark.asyncio
async def test_aprobar_usuario_ya_aprobado():
    """
...
@pytest.mark.asyncio
async def test_aprobar_usuario_email_no_verificado():
    """
    ...
    mock_user_repository = Mock(spec=IUserRepository)
    mock_user_repository.buscar_por_id = AsyncMock(return_value=unverified_user)
    mock_user_repository.actualizar = AsyncMock()
