...
@pytest.mark.asyncio
async def test_eliminar_prenda_de_pedido(mock_pedido_repository, mock_temporada_pedido_repository, mock_pedido_policy, user_registrado, temporada_activa, pedido_borrador_con_linea):
    # Arrange
    pedido_borrador, linea_de_pedido = pedido_borrador_con_linea
    mock_temporada_pedido_repository.get_temporada_activa.return_value = temporada_activa
    mock_pedido_repository.buscar_borrador_por_usuario.return_value = pedido_borrador
    mock_pedido_repository.eliminar_linea_y_recalcular.return_value = pedido_borrador
    mock_pedido_repository.guardar_pedido.return_value = pedido_borrador

    use_case = EliminarPrendaPedidoUseCase(mock_pedido_repository, mock_temporada_pedido_repository, mock_pedido_policy)

    # Act
    result = await use_case.execute(user_registrado, linea_de_pedido.id)

    # Assert
    assert isinstance(result, PedidoDTO)
    mock_pedido_repository.eliminar_linea_y_recalcular.assert_called_once_with(pedido_borrador, linea_de_pedido.id)
