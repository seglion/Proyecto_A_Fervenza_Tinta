...
class VerDetalleCuotaUseCase:
    ...
    async def execute(self, user: User, cuota_id: UUID) -> DetalleCuotaDTO:
        ...
        tipo_cuota_detalle_dto = TipoCuotaDTO.model_validate(tipo_cuota) 
        usuario_detalle_dto = UsuarioResponseDTO.model_validate(usuario) 

        return DetalleCuotaDTO(
            cuota=cuota_dto,
            temporada=temporada_dto,
            tipo_cuota_detalle=tipo_cuota_detalle_dto, 
            usuario_detalle=usuario_detalle_dto 
        )
