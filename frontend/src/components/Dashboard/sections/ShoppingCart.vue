<template>
  <AdminCard>
    <template #title>{{ t('merchandise.cartTitle') }}</template>

    <div v-if="pedidosStore.isLoading" class="text-center text-gray-400">
      {{ t('dashboard.view.feeLoading') }}
    </div>

    <div v-else-if="!pedidosStore.draftOrder || pedidosStore.cartItemCount === 0" class="text-center text-gray-400">
